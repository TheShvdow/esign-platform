...
type SectionId = 'insights' | 'users' | 'documents' | 'audit';
type ChartKey = 'docs' | 'users' | 'sigs';

@Component({
  selector: 'app-admin-page',
  standalone: true,
  imports: [CommonModule, DecimalPipe, FormsModule, RouterLink, AdminChartComponent],
  templateUrl: './admin-page.component.html',
  styleUrl: './admin-page.component.scss',
})
export class AdminPageComponent implements OnInit {
  ...
  readonly activeChart = signal<ChartKey>('docs');
  ...
  readonly docStatusKeys = [
    'DRAFT', 'PENDING_SIGNATURE', 'PARTIALLY_SIGNED',
    'FULLY_SIGNED', 'EXPIRED', 'REJECTED', 'ARCHIVED',
  ];
  ...
  readonly activeChartValues = computed(() => {
    switch (this.activeChart()) {
      case 'docs':  return this.chartDocs();
      case 'users': return this.chartUsers();
      case 'sigs':  return this.chartSigs();
    }
  });

  readonly activeChartTitle = computed(() => {
    switch (this.activeChart()) {
      case 'docs':  return 'Documents';
      case 'users': return 'Nouveaux comptes';
      case 'sigs':  return 'Signatures';
    }
  });

  readonly activeChartColor = computed(() => {
    switch (this.activeChart()) {
      case 'docs':  return '#0d9488';
      case 'users': return '#6366f1';
      case 'sigs':  return '#059669';
    }
  });

  readonly sectionLabel = computed(() => {
    switch (this.activeSection()) {
      case 'insights':  return 'Tableau de bord';
  ...
  loadStats(): void {
    ...
    this.adminApi.getStats().subscribe({
      next: (s) => this.stats.set(s),
      error: () =>
        this.statsError.set('Impossible de charger les indicateurs.'),
    });
  }
  ...
  confirmDeleteDocument(doc: DocumentDto): void {
    const ok = window.confirm(
      `Supprimer définitivement « ${doc.title} » ?\nLe fichier et les signatures seront effacés.`,
    );
    if (!ok) return;
    this.docActionBusyId.set(doc.id);
    this.docActionError.set(null);
  ...
  downloadDocument(doc: DocumentDto): void {
    this.documentService.downloadFile(doc.id, doc.originalName).subscribe({
      ...
      error: () => this.docActionError.set('Téléchargement impossible.'),
    });
  }
  ...
  formatBytes(raw: string | number): string {
    try {
      ...
      const units = ['o', 'Ko', 'Mo', 'Go', 'To'];
      let i = 0;
      while (n >= 1024n && i < units.length - 1) { n /= 1024n; i++; }
      return `${n.toString()} ${units[i]}`;
    } catch {
      return String(raw);
    }
  ...
  auditActionColor(action: string): string {
    if (action.startsWith('USER_LOGIN') || action === 'USER_REGISTER') return '#6366f1';
    if (action.includes('SIGN'))   return '#0d9488';
    if (action.includes('DELETE')) return '#dc2626';
    if (action.includes('UPLOAD')) return '#d97706';
